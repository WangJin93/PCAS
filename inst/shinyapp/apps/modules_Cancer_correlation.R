...
output$ga_cor_output <- renderPlot(width = 500,
                                   height = 300,{
                                     ...
                                     viz_corplot(df,colnames(df)[2],colnames(df)[3],method=input$cor_method,x_lab = colnames(df)[1],y_lab = colnames(df)[2])
                                    }
)
output$cor_download <- downloadHandler(
  filename = function() {
    paste0(colnames(corr_func())[1],"_",colnames(corr_func())[2],".pdf")
  },
  content = function(file) {
    df <- corr_func()

      pdf(file = file, onefile = FALSE,height = input$cor_height,width = input$cor_width)
    print(viz_corplot(df,colnames(df)[1],colnames(df)[2],method=input$cor_method,
                    x_lab = colnames(df)[1],y_lab = colnames(df)[2]))
      dev.off()
